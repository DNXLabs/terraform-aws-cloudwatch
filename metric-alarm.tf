resource "aws_cloudwatch_metric_alarm" "default" {
  ...
  unit                = try(var.filter.metric_transformation.unit, var.unit)
  ...
  metric_name        = try(var.filter.name, var.metric_name)
  namespace          = try(var.filter.metric_transformation.namespace, var.namespace)
  period             = var.period
  statistic          = var.statistic
  extended_statistic = var.extended_statistic

  dimensions = try(var.filter.metric_transformation.dimensions, var.dimensions)
